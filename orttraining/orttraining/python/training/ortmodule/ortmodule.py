...
class ORTModule(torch.nn.Module):
    ...
    def __setattr__(self, name: str, value) -> None:
        ...
        if '_torch_module' in self.__dict__ and hasattr(self.module, name):
            # If the name is an attribute of user model, update there.
            ...
            # If the name is an attribute of only ORTModule and not of the user's module, only update ORTModule
            self.__dict__[name] = value
        elif '_torch_module' in self.__dict__ or isinstance(value, torch.nn.Parameter) or \
            isinstance(value, torch.nn.Module) or isinstance(value, torch.Tensor):
            # Any attribute which is either a new attribute or a
            # torch Parameter, Module or Tensor must be set on the user's model
            _setattr_on_user_module(name, value)
        else:
